...
namespace DC365_PayrollHR.Core.Application.CommandsAndQueries.ReportsTXT
{
    ...
    public class DGTTxtQueryHandler : IDGTTxtQueryHandler
    {
        ...
        public async Task<Response<object>> CreateDGT3(int year, int month)
        {
            ...
            foreach (var item in a)
            {
                dgt3details.Add(
                    new TXTModelDGT3Detail()
                    {
                        ...
                        DocumentNumber = item.Employee.Document == null ? "" : FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.Document.DocumentNumber, 25, ' '),
                        Name = FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.Name, 50, ' '),
                        LastName = FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.LastName, 40, ' '),
                        SecondLastName = FillStringHelper.Fill(AlignDirection.Derecha, string.Empty, 40, ' '),
                        BirthDate = item.Employee.BirthDate,
                        Gender = item.Employee.Gender,
                        Salary = FillStringHelper.Fill(AlignDirection.Izquierda, item.Employee.Salary.ToString(), 16, '0'),
                        AdmissionDate = item.Employee.AdmissionDate,
                        Occupation = FillStringHelper.Fill(AlignDirection.Izquierda, item.Employee.OccupationId, 6, '0'),
                        OccupationDescription = FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.OccupationDescription, 150, ' '),
                        StartVacation = item.Employee.StartVacation,
                        EndVacation = item.Employee.EndVacation,
                        Turn = FillStringHelper.Fill(AlignDirection.Izquierda, item.Employee.Turn, 6, '0'),
                        Location = FillStringHelper.Fill(AlignDirection.Izquierda, item.Employee.LocationId, 6, '0'),
                        EductionalLevel = FillStringHelper.Fill(AlignDirection.Izquierda, item.Employee.EducationLevelId, 5, '0'),
                        Disability = FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.Turn, 50, ' '),
                    }
                );
            }
            ...
            {
                RNC = FillStringHelper.Fill(AlignDirection.Izquierda, company.Identification, 11, '0'),
                Period = period,

                Details = dgt3details,

                RegisterQty = FillStringHelper.Fill(AlignDirection.Izquierda, (2 + dgt3details.Count()).ToString(), 6, '0'),                 
            });
        }
        ...
        public async Task<Response<object>> CreateDGT4(int year, int month)
        {
            ...
            foreach (var item in a)
            {
                dgt4details.Add(
                    ...
                    {
                        ActionType = FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.NoveltyType, 3, ' '),
                        DocumentType = item.Employee.Document == null? "":TypeDocument(item.Employee.Document.DocumentType),
                        DocumentNumber = item.Employee.Document == null ? "" : FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.Document.DocumentNumber, 25, ' '),
                        Name = FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.Name, 50, ' '),
                        LastName = FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.LastName, 40, ' '),
                        ...
                        Turn = FillStringHelper.Fill(AlignDirection.Izquierda, item.Employee.Turn, 6, '0'),
                        Location = FillStringHelper.Fill(AlignDirection.Izquierda, item.Employee.LocationId, 6, '0'),
                        Nationality = FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.Nationality, 3, ' '),
            ...
            return new Response<object>(new TXTModelDGT4()
            {
                ...
                RegisterQty = FillStringHelper.Fill(AlignDirection.Izquierda, (2 + dgt4details.Count()).ToString(), 6, '0'),                 
            });
        }
        ...
        public async Task<Response<object>> CreateDGT5(int year, int month)
        {
            ...
            var employees = await _dbContext.Employees
                                            ...
                                            .Join(_dbContext.EarningCodes,
                                                ...
                                                (joins, ec) => new { Joins = joins, Ec = ec })
                                            .Where(x => x.Ec.IsUseDGT == true 
                                                   && x.Joins.Join.E.WorkStatus == WorkStatus.Empleado
                                                   && x.Joins.Join.Eh.RegisterDate >= startDate && x.Joins.Join.Eh.RegisterDate <= endDate
                                                   && x.Joins.Join.Eh.Type == "NI"
                                                   && x.Joins.Join.E.EmployeeType == EmployeeType.Contratista)
                                            .Select(x => new
                                            {
                                                x.Joins.Join.E.EmployeeId,
                                                Document = _dbContext.EmployeeDocuments.Where(y => y.EmployeeId == x.Joins.Join.E.EmployeeId).FirstOrDefault(),
            ...
            foreach (var item in a)
            {
                dgt5details.Add(
                    new TXTModelDGT5Detail()
                    {
                        DocumentType = item.Employee.Document == null ? "" : TypeDocument(item.Employee.Document.DocumentType),
                        DocumentNumber = item.Employee.Document == null ? "" : FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.Document.DocumentNumber, 25, ' '),
                        Salary = FillStringHelper.Fill(AlignDirection.Izquierda, (item.SalaryDiary * item.WorkedDays).ToString(), 16, '0'),
                        SalaryDiary = FillStringHelper.Fill(AlignDirection.Izquierda, item.SalaryDiary.ToString(), 16, '0'),
                        AdmissionDate = item.Employee.AdmissionDate.ToString("ddMMyyyy"),
                        WorkedDays = FillStringHelper.Fill(AlignDirection.Izquierda, (item.Employee.DissmisDate - item.Employee.AdmissionDate).ToString(), 2, '0'),
                        Occupation = FillStringHelper.Fill(AlignDirection.Izquierda, item.Employee.OccupationId, 6, '0'),
                        OccupationDescription = FillStringHelper.Fill(AlignDirection.Derecha, item.Employee.OccupationDescription, 150, ' '),
                        Turn = FillStringHelper.Fill(AlignDirection.Izquierda, item.Employee.Turn, 6, '0'),
                        Location = FillStringHelper.Fill(AlignDirection.Izquierda, item.Employee.LocationId, 6, '0'),
            ...
            return new Response<object>(new TXTModelDGT5()
            {
                ...
                RegisterQty = FillStringHelper.Fill(AlignDirection.Izquierda, (2 + dgt5details.Count()).ToString(), 6, '0'),
            });
        }
        ...
        public async Task<Response<object>> CreateDGT2(int year, int month)
        {
            ...
            foreach (var item in employees)
            {
                ...
                dgt2details.Add(
                    new TXTModelDGT2Detail()
                    {
                        ...
                        DocumentNumber = FillStringHelper.Fill(AlignDirection.Derecha, item.document, 25, ' '),
                        Location = FillStringHelper.Fill(AlignDirection.Izquierda, item.location, 6, '0'),
                        AmountByNormalHour = FillStringHelper.Fill(AlignDirection.Izquierda, item.SalaryDiary.ToString(), 16, '0'),
                        DayH = hours_days,
                        Reason = FillStringHelper.Fill(AlignDirection.Derecha, "d", 15, ' ')
                    }
                );
            }
            ...
            return new Response<object>(new TXTModelDGT2()
            {
                ...
                RegisterQty = FillStringHelper.Fill(AlignDirection.Izquierda, (2 + dgt2details.Count()).ToString(), 6, '0'),
            });
        }
        ...
        public async Task<Response<object>> CreateTSS(int year, int month, string payrollid, string typetss)
        {
            ...
            foreach (var item in a)
            {
                tssdetails.Add(
                    ...
                    {
                        DocumentType = TypeDocument(item.DocumentType),
                        DocumentNumber = FillStringHelper.Fill(AlignDirection.Derecha, item.DocumentNumber, 25, ' '),
                        EmployeeName = FillStringHelper.Fill(AlignDirection.Derecha, item.Name, 50, ' '),
                        EmployeeLastName = FillStringHelper.Fill(AlignDirection.Derecha, item.LastName, 40, ' '),
                        EmployeeSecondLastName = FillStringHelper.Fill(AlignDirection.Derecha, "", 40, ' '),
                        BirthDate = item.BirthDate,
                        Gender = item.Gender,
                        Salary = FillStringHelper.Fill(AlignDirection.Izquierda, item.TotalAmount.ToString(), 16, '0'),
                        Salary_ISR = FillStringHelper.Fill(AlignDirection.Izquierda, item.TotalTaxAmount.ToString(), 16, '0'),
                        EmptyAmount = FillStringHelper.Fill(AlignDirection.Izquierda, "0.00", 16, '0'),
                    }
                );
            }
            ...
            return new Response<object>(new TXTModelTSS()
            {
                ...
                RegisterQty = FillStringHelper.Fill(AlignDirection.Izquierda, (2 + tssdetails.Count()).ToString(), 6, '0'),
            });
        }
        ...
        public async Task<Response<object>> CreatePayroll(string payrollprocessid, string payrollid)
        {
            ...
            var employees = await _dbContext.PayrollsProcess
                                            ...
                                            .Where(x => x.Join.Pp.PayrollProcessId == payrollprocessid
                                                   && x.Join.Pp.PayrollProcessStatus == PayrollProcessStatus.Pagado
                                                   && x.Join.Pp.PayrollId == payrollid
                                                   && x.Join.Ppd.PayMethod == PayMethod.Transferencia)
                                            .Select(x => new
                                            {
                                                Document = _dbContext.EmployeeDocuments.Where(y => y.EmployeeId == x.E.EmployeeId && y.IsPrincipal == true).FirstOrDefault(),
                                                Contact = _dbContext.EmployeeContactsInf.Where(y => y.EmployeeId == x.E.EmployeeId && y.IsPrincipal == true && y.ContactType == ContactType.Correo).FirstOrDefault(),
            ...
            foreach (var item in employees)
            {
                ...
                payrolldetails.Add(
                    ...
                    {
                        RNC = FillStringHelper.Fill(AlignDirection.Derecha, company.Identification, 15, ' '),
                        Sequence = FillStringHelper.Fill(AlignDirection.Izquierda, banksecuence.ToString(), 7, '0'),
                        TransactionSequence = FillStringHelper.Fill(AlignDirection.Izquierda, cont.ToString(), 7, '0'),
                        ToAccount = FillStringHelper.Fill(AlignDirection.Derecha,(item.Bank == null?"": item.Bank.AccountNum), 20, ' '),
                        ToAccountType = FillStringHelper.Fill(AlignDirection.Derecha, (item.Bank == null ? "" : TypeBankAccountPayroll(item.Bank.AccountType)),1, ' '),
                        TransactionAmount = FillStringHelper.Fill(AlignDirection.Izquierda, item.TotalAmount.ToString().Replace(".", ""), 13, '0'),
                        IdentificationType = item.Document == null ? "xx":TypeDocumentPayroll(item.Document.DocumentType),
                        IdentificationNumber = FillStringHelper.Fill(AlignDirection.Derecha, (item.Document == null ? "" : item.Document.DocumentNumber), 15, ' '),
                        EmployeeName = FillStringHelper.Fill(AlignDirection.Derecha, employeename, 35, ' '),
                        ReferenceNumber = FillStringHelper.Fill(AlignDirection.Derecha, payrollprocessid.Substring(5, 9), 12, ' '),
                        Description = FillStringHelper.Fill(AlignDirection.Derecha, "PAGO NOMINA", 40, ' '),
                        DueDate = FillStringHelper.Fill(AlignDirection.Derecha, "", 4, ' '),
                        ContactForm = "1",
                        EmployeeEmail = FillStringHelper.Fill(AlignDirection.Derecha, (item.Contact == null?"":item.Contact.NumberAddress), 40, ' '),
                        EmployeePhone = FillStringHelper.Fill(AlignDirection.Derecha, "", 12, ' '),
                        PaymentProcess = "00",
                        EmptyValue = FillStringHelper.Fill(AlignDirection.Derecha, "", 27, ' '),
                        Filler = FillStringHelper.Fill(AlignDirection.Derecha, "", 52, ' ')
                    }
                );
            ...
            {
                RNC = FillStringHelper.Fill(AlignDirection.Derecha, company.Identification, 11, ' '),
                CompanyName = FillStringHelper.Fill(AlignDirection.Derecha, company.Name, 35, ' '),
                ServiceType = "01",
                EfectiveDate = employees.First().PaymentDate.ToString("yyyyMMdd"),
                QtyDebit = FillStringHelper.Fill(AlignDirection.Izquierda, "0", 11, '0'),
                TotalAmountDebit = FillStringHelper.Fill(AlignDirection.Izquierda, "0", 13, '0'),
                QtyCredit = FillStringHelper.Fill(AlignDirection.Izquierda, payrolldetails.Count().ToString(), 11, '0'),
                TotalAmountCredit = FillStringHelper.Fill(AlignDirection.Izquierda, employees.Sum(x=> x.TotalAmount).ToString().Replace(".", ""), 13, '0'),
                Hour = DateTime.Now.ToString("HHMM"),
                Email = FillStringHelper.Fill(AlignDirection.Derecha, company.Email, 40, ' ') ,
                Status = "",
                Filler = FillStringHelper.Fill(AlignDirection.Derecha, "", 136, ' '),
