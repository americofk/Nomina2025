/// <summary>
/// Servicio para la gestión de vacantes.
/// Administra las vacantes y posiciones abiertas para reclutamiento.
/// </summary>
...
{
    /// <summary>
    /// Servicio de proceso para ProcessVacants.
    /// </summary>
    public class ProcessVacants : ServiceBase
    {
        ...
        public async Task<IEnumerable<Vacants>> GetAllDataAsync(string PropertyName = "", string PropertyValue = "", int _PageNumber = 1, int PageSize=1500)
        {
        ...
        /// <summary>
        /// Obtiene vacantes paginados con total de registros.
        /// </summary>
        public async Task<PagedResult<Vacants>> GetAllDataPagedAsync(string PropertyName = "", string PropertyValue = "", int pageNumber = 1, int pageSize = 20)
        {
            var result = new PagedResult<Vacants>
            {
                PageNumber = pageNumber,
                PageSize = pageSize,
                Data = new List<Vacants>()
            };

            string urlData = $"{urlsServices.GetUrl("Vacants")}?PageNumber={pageNumber}&PageSize={pageSize}&PropertyName={PropertyName}&PropertyValue={PropertyValue}";

            var Api = await ServiceConnect.connectservice(Token, urlData, null, HttpMethod.Get);

            if (Api.IsSuccessStatusCode)
            {
                var response = JsonConvert.DeserializeObject<PagedResponse<List<Vacants>>>(Api.Content.ReadAsStringAsync().Result);
                if (response != null)
                {
                    result.Data = response.Data ?? new List<Vacants>();
