...
/// Administra el catálogo de instructores para cursos de capacitación.
/// </summary>
...
{
    /// <summary>
    /// Servicio de proceso para ProcessInstructor.
    /// </summary>
    public class ProcessInstructor: ServiceBase
    {
        ...
        public async Task<IEnumerable<Instructor>> GetAllDataAsync(string PropertyName = "", string PropertyValue = "", int _PageNumber = 1, int PageSize=1500)
        {
        ...
        /// <summary>
        /// Obtiene instructores paginados con total de registros.
        /// </summary>
        public async Task<PagedResult<Instructor>> GetAllDataPagedAsync(string PropertyName = "", string PropertyValue = "", int pageNumber = 1, int pageSize = 20)
        {
            var result = new PagedResult<Instructor>
            {
            ...
            string urlData = $"{urlsServices.GetUrl("Instructors")}?PageNumber={pageNumber}&PageSize={pageSize}&PropertyName={PropertyName}&PropertyValue={PropertyValue}";

            var Api = await ServiceConnect.connectservice(Token, urlData, null, HttpMethod.Get);

            if (Api.IsSuccessStatusCode)
            {
                var response = JsonConvert.DeserializeObject<PagedResponse<List<Instructor>>>(Api.Content.ReadAsStringAsync().Result);
                if (response != null)
                {
                    result.Data = response.Data ?? new List<Instructor>();
        ...
        public async Task<ResponseUI> PostDataAsync(Instructor _model)
        {
            ...
            if (Api.IsSuccessStatusCode)
            {
                ...
                else
                {
                    ...
                    responseUI.IdType = DataApi.Data?.InstructorId;
                }
