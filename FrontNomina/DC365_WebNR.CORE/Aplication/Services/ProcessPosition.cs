...
namespace DC365_WebNR.CORE.Aplication.Services
{
    ...
    public class ProcessPosition : ServiceBase
    {
        ...
        /// <summary>
        /// Obtiene puestos paginados con total de registros.
        /// </summary>
        public async Task<PagedResult<Position>> GetAllDataPagedAsync(string PropertyName = "", string PropertyValue = "", int pageNumber = 1, int pageSize = 20)
        {
            ...
            string urlData = $"{urlsServices.GetUrl("PositionsEnabled")}?PageNumber={pageNumber}&PageSize={pageSize}&PropertyName={PropertyName}&PropertyValue={PropertyValue}";

            var Api = await ServiceConnect.connectservice(Token, urlData, null, HttpMethod.Get);

            if (Api.IsSuccessStatusCode)
            {
                var response = JsonConvert.DeserializeObject<PagedResponse<List<Position>>>(Api.Content.ReadAsStringAsync().Result);
                if (response != null)
                {
                    result.Data = response.Data ?? new List<Position>();
