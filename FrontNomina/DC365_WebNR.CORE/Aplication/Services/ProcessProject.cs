...
namespace DC365_WebNR.CORE.Aplication.Services
{
    ...
    public class ProcessProject : ServiceBase
    {
        ...
        public async Task<IEnumerable<Project>> GetAllDataAsync(string PropertyName = "", string PropertyValue = "", int _PageNumber = 1,int PageSize=1500)
        {
        ...
        /// <summary>
        /// Obtiene proyectos paginados con total de registros.
        /// </summary>
        public async Task<PagedResult<Project>> GetAllDataPagedAsync(string PropertyName = "", string PropertyValue = "", int pageNumber = 1, int pageSize = 20)
        {
            ...
            string urlData = $"{urlsServices.GetUrl("ProjectsEnabled")}?PageNumber={pageNumber}&PageSize={pageSize}&PropertyName={PropertyName}&PropertyValue={PropertyValue}";

            var Api = await ServiceConnect.connectservice(Token, urlData, null, HttpMethod.Get);

            if (Api.IsSuccessStatusCode)
            {
                var response = JsonConvert.DeserializeObject<PagedResponse<List<Project>>>(Api.Content.ReadAsStringAsync().Result);
                if (response != null)
                {
                    result.Data = response.Data ?? new List<Project>();
        ...
        public async Task<ResponseUI> PostDataAsync(Project _model)
        {
            ...
            {                
                ...
                responseUI.IdType = DataApi.Data?.ProjId;
            }
            else
            {
