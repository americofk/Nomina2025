...
namespace DC365_WebNR.CORE.Aplication.Services
{
    ...
    public class ProcessProjCategory : ServiceBase
    {
        ...
        /// <summary>
        /// Obtiene categorias de proyectos paginadas con total de registros.
        /// </summary>
        public async Task<PagedResult<ProjCategory>> GetAllDataPagedAsync(string PropertyName = "", string PropertyValue = "", int pageNumber = 1, int pageSize = 20)
        {
            var result = new PagedResult<ProjCategory>
            {
                PageNumber = pageNumber,
                PageSize = pageSize,
                Data = new List<ProjCategory>()
            };

            string urlData = $"{urlsServices.GetUrl("ProjCategoryEnabled")}?PageNumber={pageNumber}&PageSize={pageSize}&PropertyName={PropertyName}&PropertyValue={PropertyValue}";

            var Api = await ServiceConnect.connectservice(Token, urlData, null, HttpMethod.Get);

            if (Api.IsSuccessStatusCode)
            {
                var response = JsonConvert.DeserializeObject<PagedResponse<List<ProjCategory>>>(Api.Content.ReadAsStringAsync().Result);
                if (response != null)
                {
                    result.Data = response.Data ?? new List<ProjCategory>();
