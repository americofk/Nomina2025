...
namespace DC365_WebNR.CORE.Aplication.Services
{
    ...
    public class ProcessClassRoom: ServiceBase
    {
        ...
        /// <summary>
        /// Obtiene salones de cursos paginados con total de registros.
        /// </summary>
        public async Task<PagedResult<ClassRoom>> GetAllDataPagedAsync(string PropertyName = "", string PropertyValue = "", int pageNumber = 1, int pageSize = 20)
        {
            var result = new PagedResult<ClassRoom>
            {
            ...
            string urlData = $"{urlsServices.GetUrl("ClassRoom")}?PageNumber={pageNumber}&PageSize={pageSize}&PropertyName={PropertyName}&PropertyValue={PropertyValue}";

            var Api = await ServiceConnect.connectservice(Token, urlData, null, HttpMethod.Get);

            if (Api.IsSuccessStatusCode)
            {
                var response = JsonConvert.DeserializeObject<PagedResponse<List<ClassRoom>>>(Api.Content.ReadAsStringAsync().Result);
                if (response != null)
                {
                    result.Data = response.Data ?? new List<ClassRoom>();
        ...
        public async Task<ResponseUI> PostDataAsync(ClassRoom _model)
        {
            ...
            if (Api.IsSuccessStatusCode)
            {
                ...
                else
                {
                    ...
                    responseUI.IdType = DataApi.Data?.ClassRoomId;
                }
