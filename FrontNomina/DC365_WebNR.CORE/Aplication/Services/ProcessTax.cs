...
namespace DC365_WebNR.CORE.Aplication.Services
{
    ...
    public class ProcessTax: ServiceBase
    {
        ...
        /// <summary>
        /// Obtiene impuestos paginados con total de registros.
        /// </summary>
        public async Task<PagedResult<Tax>> GetAllDataPagedAsync(string PropertyName = "", string PropertyValue = "", int pageNumber = 1, int pageSize = 20)
        {
            var result = new PagedResult<Tax>
            {
            ...
            string urlData = $"{urlsServices.GetUrl("Taxes​Enabled")}?PageNumber={pageNumber}&PageSize={pageSize}&PropertyName={PropertyName}&PropertyValue={PropertyValue}";

            var Api = await ServiceConnect.connectservice(Token, urlData, null, HttpMethod.Get);

            if (Api.IsSuccessStatusCode)
            {
                var response = JsonConvert.DeserializeObject<PagedResponse<List<Tax>>>(Api.Content.ReadAsStringAsync().Result);
                if (response != null)
                {
                    result.Data = response.Data ?? new List<Tax>();
