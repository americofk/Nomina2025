...
namespace DC365_WebNR.CORE.Aplication.Services
{
    ...
    public class ProcessPayrollProcess : ServiceBase
    {
        ...
        /// <summary>
        /// Obtiene procesos de nomina paginados con total de registros.
        /// </summary>
        public async Task<PagedResult<PayrollProcess>> GetAllDataPagedAsync(string PropertyName = "", string PropertyValue = "", int pageNumber = 1, int pageSize = 20)
        {
            var result = new PagedResult<PayrollProcess>
            {
                PageNumber = pageNumber,
                PageSize = pageSize,
                Data = new List<PayrollProcess>()
            };

            string urlData = $"{urlsServices.GetUrl("PayrollProcess")}?PageNumber={pageNumber}&PageSize={pageSize}&PropertyName={PropertyName}&PropertyValue={PropertyValue}";

            var Api = await ServiceConnect.connectservice(Token, urlData, null, HttpMethod.Get);

            if (Api.IsSuccessStatusCode)
            {
                var response = JsonConvert.DeserializeObject<PagedResponse<List<PayrollProcess>>>(Api.Content.ReadAsStringAsync().Result);
                if (response != null)
                {
                    result.Data = response.Data ?? new List<PayrollProcess>();
