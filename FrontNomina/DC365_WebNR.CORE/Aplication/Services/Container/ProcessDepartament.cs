...
namespace DC365_WebNR.CORE.Aplication.Services.Container
{
    ...
    public class ProcessDepartament : ServiceBase
    {
        ...
        public async Task<IEnumerable<Department>> GetAllDataAsync(string PropertyName = "", string PropertyValue = "", int _PageNumber = 1,int PageSize=1500)
        {
        ...
        /// <summary>
        /// Obtiene departamentos con informacion de paginacion.
        /// </summary>
        public async Task<PagedResult<Department>> GetAllDataPagedAsync(string PropertyName = "", string PropertyValue = "", int pageNumber = 1, int pageSize = 20)
        {
            ...
            string urlData = $"{urlsServices.GetUrl("Departments")}?PageNumber={pageNumber}&PageSize={pageSize}&PropertyName={PropertyName}&PropertyValue={PropertyValue}";

            var Api = await ServiceConnect.connectservice(Token, urlData, null, HttpMethod.Get);

            if (Api.IsSuccessStatusCode)
            {
                var response = JsonConvert.DeserializeObject<PagedResponse<List<Department>>>(Api.Content.ReadAsStringAsync().Result);
                if (response != null)
                {
                    result.Data = response.Data ?? new List<Department>();
