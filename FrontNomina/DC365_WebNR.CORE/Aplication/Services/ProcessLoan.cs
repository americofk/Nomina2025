...
namespace DC365_WebNR.CORE.Aplication.Services
{
    ...
    public class ProcessLoan: ServiceBase
    {
        ...
        public async Task<IEnumerable<Loan>> GetAllDataAsync(string PropertyName="", string PropertyValue="", int _PageNumber = 1, int PageSize=1500)
        {
        ...
        /// <summary>
        /// Obtiene prestamos paginados con total de registros.
        /// </summary>
        public async Task<PagedResult<Loan>> GetAllDataPagedAsync(string PropertyName = "", string PropertyValue = "", int pageNumber = 1, int pageSize = 20)
        {
            var result = new PagedResult<Loan>
            {
            ...
            string urlData = $"{urlsServices.GetUrl("Loans")}?PageNumber={pageNumber}&PageSize={pageSize}&PropertyName={PropertyName}&PropertyValue={PropertyValue}";

            var Api = await ServiceConnect.connectservice(Token, urlData, null, HttpMethod.Get);

            if (Api.IsSuccessStatusCode)
            {
                var response = JsonConvert.DeserializeObject<PagedResponse<List<Loan>>>(Api.Content.ReadAsStringAsync().Result);
                if (response != null)
                {
                    result.Data = response.Data ?? new List<Loan>();
        ...
        public async Task<ResponseUI> PostDataAsync(Loan _model)
        {
            ...
            if (Api.IsSuccessStatusCode)
            {
                ...
                responseUI.IdType = DataApi.Data?.LoanId;
            }
            else
            {
